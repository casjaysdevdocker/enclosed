...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202509161147-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  entrypoint.sh --help
# @@Copyright        :  Copyright: (c) 2025 Jason Hempstead, Casjays Developments
# @@Created          :  Tuesday, Sep 16, 2025 11:47 EDT
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for enclosed
...
SERVICE_USER="${SERVICE_USER:-$enclosed}"  # execute command as another user
SERVICE_GROUP="${SERVICE_GROUP:-enclosed}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for enclosed"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
